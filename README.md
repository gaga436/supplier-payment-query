...
> **AI 驱动的财务智能助手** — 基于博西家电（BSH Hausgeräte）供应商数据
...
- **演示数据** — 27条博西家电供应商付款记录（基于公开信息）（加西贝拉、宝钢、巴斯夫、SAP等），总计 **¥2.79亿**
...
| 🎯 **博西家电供应商数据** | 27条供应商付款记录（演示），涵盖IT、采购、物流、市场等6大部门 |
...
微信/WeChat ─┼──→ Web UI ──→ FastAPI ──→ QueryEngine ──→ DeepSeek API
             │                   │                      └→ 规则降级
             │                   ↓
             └── Hermes Gateway  SQLite → 27条BSH付款记录（演示）
```
...
├── data/
│   ├── generate_real_data.py # BSH 演示数据生成器
│   ├── payments_real.csv    # 演示数据 CSV
